...
- URL source resolver hints such as `direct-media`, `provider:reddit`, `provider:youtube`, `metadata`, and `iframe`
...
Runtime URL feeds resolve source metadata on demand from user-provided links. Resolution may use the pasted URL directly as media, a known provider adapter/embed such as Reddit or YouTube, runtime-only page metadata, or a static iframe fallback. Local image, video, and audio uploads use browser object URLs for the current session only and are revoked on cleanup.
