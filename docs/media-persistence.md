...
- User-pasted Reddit post permalinks in feed configuration settings
...
Runtime feeds fetch source metadata on demand from user-provided post links. Local image, video, and audio uploads use browser object URLs for the current session only and are revoked on cleanup.
