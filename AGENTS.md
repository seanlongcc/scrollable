...
- Reddit runtime fetching intentionally does not use Reddit OAuth credentials. Server runtime skips public Reddit JSON by default for both listing URLs and direct post URLs because hosted deployments are commonly blocked; set non-secret `REDDIT_ENABLE_PUBLIC_JSON=1` only when intentionally testing the old JSON path. Runtime fallbacks use old Reddit/Redlib HTML and Reddit RSS. RSS fallback resolves Reddit-hosted `v.redd.it` videos through Reddit's public `redditmedia.com/mediaembed/<postId>` endpoint, Reddit gallery links through old Reddit gallery HTML, and external provider links such as Redgifs through the bundled runtime `yt-dlp` extractor. All paths are runtime-only.
- Optional server-only `REDDIT_REDLIB_ORIGIN` overrides the last-resort Redlib origin(s) used when Reddit JSON/RSS and old Reddit are forbidden for pasted post URLs or listing URLs. Redlib fallback must convert media links back to original Reddit CDN URLs and remain runtime-only. Browser/workbench Reddit runtime fetching must call the app-owned `GET /api/reddit/listing` route only; do not add browser-side Reddit `.json` or JSONP fallback because Reddit is deprecating unauthenticated JSON access and browsers now surface 403 MIME/type errors in third-party contexts.
- Redlib listing origins can return post blocks with different media markup. Listing fallback must prefer an origin with enough supported media, or the highest supported-media count after candidates settle, not the first origin that merely returns post blocks.
- Redlib direct-post origins can return metadata-only pages before a slower origin returns gallery media. Direct-post fallback must keep racing origins until it finds supported media, with metadata-only pages as last-resort metadata only.
- Reddit listing fetches try old Reddit/Redlib HTML before RSS by default. This preserves album/listing counts on deployments where RSS is partial. Set non-secret `REDDIT_LISTING_HTML_FALLBACK_FIRST=0` only when intentionally testing the older RSS-before-HTML order.
- Reddit direct post fetches also try old Reddit/Redlib HTML before RSS by default. This prevents partial direct-post RSS feeds from truncating albums before stronger album HTML fallbacks run. Reddit URL parsing can recover valid embedded Reddit source URLs from pasted/glued text, for example when an invalid subreddit fragment is accidentally joined to a direct post URL.
