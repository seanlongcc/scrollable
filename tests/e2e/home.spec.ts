...
test("home renders multi-view wall without media previews", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "scrollable.app" })).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "Open layouts" })).toBeVisible();
...
test("library page shows metadata-only setup state", async ({ page }) => {
  ...
  await expect(
    page.getByText(/Supabase env missing|Account library locked|Sign in/i).first(),
  ).toBeVisible();
  await expect(page.locator("img, video")).toHaveCount(0);
});
